    ...
    <>
      <div className="mb-6 border-b border-[var(--border)] pb-5">
        ...
        <p className="mt-1 text-sm text-[var(--muted)]">
          A quick view of completed achievements and overall usage progress across the workspace.
        </p>
      </div>
      ...
      <div className="glass-card mb-6">
        <div className="flex items-center gap-4">
          <div className="flex h-16 w-16 items-center justify-center rounded-2xl text-3xl"
               style={{ background: 'rgba(180,109,82,0.1)', border: '1px solid rgba(180,109,82,0.2)' }}>
            🧭
          </div>
          <div>
            <div className="text-2xl font-bold text-[var(--text-strong)]">
              {totalUnlocked} <span className="text-base font-normal text-[var(--muted)]">/ {total} milestones</span>
            </div>
            <div className="text-sm text-[var(--muted)]">
              {totalUnlocked === total
                ? 'All available achievements are unlocked.'
                : `${total - totalUnlocked} more achievements remain to unlock.`}
            </div>
          </div>
          <div className="ml-auto hidden sm:block" style={{ width: 120 }}>
            <div className="h-2 w-full rounded-full" style={{ background: 'rgba(123,92,71,0.12)' }}>
              <div
                className="h-full rounded-full"
                style={{
                  width: `${(totalUnlocked / total) * 100}%`,
                  background: 'linear-gradient(90deg, #d8aa74, #7c5a3d)',
      ...
      <div className="mb-6">
        <StreakHeatmap activity={gameState.activity || {}} />
      </div>
        ...
        {Object.values(ACHIEVEMENTS).map((ach) => {
          ...
          return (
            <div
              ...
              style={
                unlocked
                  ? { borderColor: 'rgba(180,109,82,0.3)', boxShadow: '0 14px 28px rgba(124,90,61,0.1)' }
                  : {}
              }
            >
              ...
              <div
                ...
                style={
                  unlocked
                    ? { background: 'rgba(95,127,82,0.1)', color: 'var(--neon-green)', border: '1px solid rgba(95,127,82,0.2)' }
                    : { background: 'rgba(100,116,139,0.1)', color: 'var(--muted)', border: '1px solid rgba(100,116,139,0.15)' }
                }
              >
                {unlocked ? `Unlocked • +${ach.xp} XP` : `Locked • +${ach.xp} XP`}
              </div>
            </div>
          )
        })}
